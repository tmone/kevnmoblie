...
var app = new Framework7({
  ...
  methods: {
    ...
    updateStore: function () {
      //debugger;
      app.store = new DevExpress.data.CustomStore({
        //key: "Consignm",
        load: function () {
          $.ajax({
      ...
      if (app.gridComponent) {
        if(exact){
          ...
          app.store.load().done(function (as){
            var t = as.filter(function(x){return x.Consignment_No.endsWith(a);});
            if(t.length==1){
    ...
    codePush.sync(null,
      {
        ...
        deploymentKey: "XnUGvztT-lpyfu-TcJdqCis4xdWmH1HyBhOkE"
